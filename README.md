...
- **One shared surface.** The evidence panel records each registered-tool `execute` callback, its input, and its bounded result inside the page the researcher is already reading. It makes callback activity inspectable without claiming to identify the caller.
